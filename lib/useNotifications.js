...
import { isPersonalProject } from './personal';
...
export function useNotifications({
    ...
    const rawNotifications = useMemo(() => {
        ...
        tasks.forEach(t => {
            ...
            const taskAuthor = t.authorId || t.author_id || t.createdBy || t.created_by;
            const isCreatorMe = taskAuthor ? isMatchingUser(taskAuthor) : false;
            ...
            // Tugas hanya dianggap ditugaskan ke saya oleh orang lain jika:
            // 1. PIC adalah saya, DAN ada pembuat tugas (taskAuthor) yang BUKAN saya
            // 2. ATAU tugas ini di-share ke saya dan pembuatnya bukan saya
            const isAssignedByOther = isAssignedToMe && taskAuthor && !isCreatorMe;
            const isSharedByOther = isSharedToMe && !isCreatorMe;

            if ((isAssignedByOther || isSharedByOther) && t.status !== 'Done') {
                const authorMember = members.find(m => (m.id === taskAuthor) && !isMatchingUser(m.id));
                const proj = projects.find(p => p.id === t.projectId);
                list.push({
                    id: `task-shared-${t.id}`,
                    category: 'shared',
            ...
            if (Array.isArray(t.todos)) {
                t.todos.forEach((todo, idx) => {
                    ...
                    if (isTodoForMe && !todo.done && !isCreatorMe && (taskAuthor || !isAssignedToMe)) {
                        list.push({
        ...
        // 3. Notifikasi Jadwal (Khusus Jadwal Meeting / Rapat; Worksheet dikecualikan karena pekerjaan rutin)
        schedules.forEach(s => {
            const isMeeting = isMeetingSchedule(s);
            // Worksheet tidak perlu masuk notifikasi karena merupakan pekerjaan rutin
            if (!isMeeting) return;
            ...
            if (isToday && (isPic || isAttendee || isShared)) {
                ...
                list.push({
                    ...
                    severity: 'indigo',
                    priorityOrder: 2,
                    title: `Jadwal Rapat Hari Ini: "${s.title}"`,
                    description: `Pukul ${timeStr}${s.location ? ` • Lokasi: ${s.location}` : ''}. Agenda rapat Anda hari ini (${s.day}).`,
                    timeLabel: timeStr,
            ...
            if (!isPic && (isAttendee || isShared)) {
                ...
                list.push({
                    ...
                    severity: 'indigo',
                    priorityOrder: 3,
                    title: `Undangan Rapat Dibagikan: "${s.title}"`,
        ...
        if (userLevel >= 2) {
            tasks.forEach(t => {
                ...
                // Task di workspace pribadi tidak boleh dieskalasi ke atasan (hanya untuk pemiliknya)
                const radarProject = projects.find(p => p.id === t.projectId);
                if (isPersonalProject(radarProject)) return;
