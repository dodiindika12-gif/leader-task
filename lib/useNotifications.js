...
export function useNotifications({
    ...
    // Storage Keys
    const storageKeyRead = useMemo(() => `task_leader_read_notifications_${activeUserId || 'default'}`, [activeUserId]);
    const storageKeyDismissed = useMemo(() => `task_leader_dismissed_notifications_${activeUserId || 'default'}`, [activeUserId]);
    ...
    // Otomatis tandai dilihat saat user membuka menu atau submenu tertentu
    const markMenuAsViewed = useCallback((menuKey) => {
        if (!menuKey) return;
        const itemsToMark = unreadNotifications.filter(n =>
            n.menuKey === menuKey || n.parentMenuKey === menuKey
        );
        if (itemsToMark.length === 0) return;
        const next = new Set(readIds);
        itemsToMark.forEach(n => next.add(n.id));
