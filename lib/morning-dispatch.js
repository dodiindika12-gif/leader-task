...
import { isPersonalProject } from "./personal.js";
...
export function buildTaskNotificationMessage({ member, members, projects, tasks, todayYMD, todayHuman }) {
  ...
  // Task di workspace pribadi tidak boleh dieskalasi / masuk radar atasan
  const notPersonalTask = (t) => !isPersonalProject(projectMap.get(t.project_id));

  // 1. Task milik member sendiri (Direct PIC)
  const myTasks = tasks.filter((t) => t.pic_id === member.id && notPersonalTask(t));
  ...
  const subTasks = tasks.filter((t) => subIds.has(t.pic_id) && notPersonalTask(t));
...
export async function runMorningDispatch({
  ...
  const [
    ...
    client
      ...
      .eq("is_active", true),
    client.from("projects").select("id, name, division, description, owner_id"),
    client
      .from("tasks")
